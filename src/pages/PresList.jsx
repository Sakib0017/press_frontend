...
export default function PresList() {
  ...
  return (
    <div className="flex flex-col lg:flex-row h-screen overflow-hidden bg-slate-50 text-slate-700 font-sans antialiased">
      <Navbar />
      <div className="flex-1 flex flex-col min-w-0 overflow-y-auto lg:overflow-hidden">
        ...
        <div className="bg-white border-b border-slate-200 p-4 shadow-sm">
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-8 gap-3 items-end">
            <div><label className="block text-[10px] font-bold text-slate-500 mb-1">From Date</label><input type="date" value={filters.from_date} onChange={e=>setFilters({...filters, from_date:e.target.value})} className="w-full border border-slate-300 rounded px-2 py-1.5 text-xs" /></div>
            <div><label className="block text-[10px] font-bold text-slate-500 mb-1">To Date</label><input type="date" value={filters.to_date} onChange={e=>setFilters({...filters, to_date:e.target.value})} className="w-full border border-slate-300 rounded px-2 py-1.5 text-xs" /></div>
            <div><label className="block text-[10px] font-bold text-slate-500 mb-1">Doctor</label><div className="w-full border border-slate-300 rounded px-2 py-1.5 text-xs bg-slate-50 text-slate-700 font-medium">{doctor?.name}<input type="hidden" value={doctor?.name} /></div></div>
            <div><label className="block text-[10px] font-bold text-slate-500 mb-1">Prescription ID</label><input type="text" value={filters.prescription_id} onChange={e=>setFilters({...filters, prescription_id:e.target.value})} placeholder="#ID" className="w-full border border-slate-300 rounded px-2 py-1.5 text-xs" /></div>
            <div><label className="block text-[10px] font-bold text-slate-500 mb-1">Patient Name</label><input type="text" value={filters.patient_name} onChange={e=>setFilters({...filters, patient_name:e.target.value})} placeholder="Patient Name" className="w-full border border-slate-300 rounded px-2 py-1.5 text-xs" /></div>
            <div className="sm:col-span-2 lg:col-span-3 flex flex-wrap gap-2 items-end">
              <button onClick={fetchData} className="bg-[#337ab7] hover:bg-blue-700 text-white px-4 py-1.5 rounded text-xs font-bold flex items-center gap-1 shrink-0">Search</button>
              <button onClick={()=>{ setFilters({ from_date: new Date().toISOString().slice(0,10), to_date: new Date().toISOString().slice(0,10), patient_name:'', prescription_id:''}); }} className="border border-orange-400 text-orange-500 hover:bg-orange-50 px-4 py-1.5 rounded text-xs font-bold">Reset</button>
              <span className="text-[10px] font-bold text-slate-400 ml-auto self-center">{prescriptions.length} result(s)</span>
            </div>
          </div>
        </div>
